    ...
    public class AslUserCOController : AppController
    {
        ...
        public ActionResult Create(AslUserco asluserco)
        {
            if (ModelState.IsValid)
            {
                ...
                {
                    Int64 maxData = Convert.ToInt64((from n in db.AslUsercoDbSet where asluserco.COMPID == n.COMPID && n.BRANCHID==asluserco.BRANCHID select n.USERID).Max());

                    Int64 R = Convert.ToInt64(asluserco.BRANCHID+"999");

                    if (maxData == 0)
                    {
                        asluserco.OPTP = "CompanyAdmin";
                        asluserco.USERID = Convert.ToInt64(asluserco.BRANCHID + "001");
                        ...
                        foreach (ASL_MENU a in qrM)
                        {
                            ASL_ROLE role = new ASL_ROLE()
                            {
                                ...
                                USERID = Convert.ToInt64(asluserco.BRANCHID + "001"),
                                MODULEID = a.MODULEID,
                                MENUID = a.MENUID,
                                MENUTP = a.MENUTP,
                                SERIAL = a.SERIAL,
        ...
        public JsonResult GetUserInformation(Int64 changedDropDown)
        {
            ...
            String Branch="",userName = "", departmentName = "", operationType = "", address = "", mobileNo = "", emailId = "", loginBy = "", loginId = "", loginPassword = "", timerFor = "", timerto = "", status = "", inserttime = "", insertIpno = "", insltude = "", userpc = "";
            ...
            foreach (var n in rt)
            {
                ...
                Branch = Convert.ToString(n.branch);
            }
