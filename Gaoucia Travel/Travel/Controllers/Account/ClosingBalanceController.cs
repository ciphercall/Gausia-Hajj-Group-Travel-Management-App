    ...
    public class ClosingBalanceController : AppController
    {
            ...
            if (command == "Submit")
            {
                if (model.AGlMaster.TRANSDT != null)
                {
                   ...
                   // string converttoString1 = Convert.ToString(changedtxt.ToString("dd-MM-yyyy"));
                    ...
                    var result = (from d in db.GlMasterDbSet where d.TRANSTP == "OPEN" && d.COMPID == companyID select d).ToList();
                    if (result.Count == 0)
                    {
                    ...
                    else
                    {
                        ...
                        DateTime transtp=new DateTime();
                        foreach (var ss in result)
                        {
                            transtp = Convert.ToDateTime(ss.TRANSDT);
                        }
                        if (transtp == model.AGlMaster.TRANSDT)
                        {
                            TempData["message"] = "Continue";
                            TempData["costpool"] = model;
                            TempData["TRANSDT"] = converttoString;
                ...
                if (model.AGlMaster.TRANSDT != null && (model.AGlMaster.DEBITCD!=0 && model.AGlMaster.DEBITCD!=null))
                {
                    ...
                    System.Data.SqlClient.SqlConnection conn =
                        new System.Data.SqlClient.SqlConnection(
                            System.Configuration.ConfigurationManager.ConnectionStrings["TravelDbContext"].ToString());
                    string query =
                        string.Format(
        ...
        public JsonResult TagSearch(string term)
        {
            ...
            var tags = from p in db.GlAcchartDbSet
                       where p.COMPID == compid && p.HLEVELCD==5
                       select p.ACCOUNTNM;
